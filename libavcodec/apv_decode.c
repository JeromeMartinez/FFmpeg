...
{
    int err, bit_depth, dimensions_changed = 0;
    enum AVPixelFormat pix_fmt;
    ...
    pix_fmt = apv_format_table[header->frame_info.chroma_format_idc][bit_depth - 4 >> 2];
    if (!pix_fmt) {
    ...
    if (pix_fmt != apv->pix_fmt || dimensions_changed) {
        apv->pix_fmt = pix_fmt;
