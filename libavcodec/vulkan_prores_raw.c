...
static int vk_prores_raw_end_frame(AVCodecContext *avctx)
{
    ...
    FFVulkanShader *idct_shader = &prv->idct[prr->version];
...
static void vk_decode_prores_raw_uninit(FFVulkanDecodeShared *ctx)
{
    ...
    ff_vk_shader_free(&ctx->s, &fv->idct[0]);
    ff_vk_shader_free(&ctx->s, &fv->idct[1]);
...
static int vk_decode_prores_raw_init(AVCodecContext *avctx)
{
    ...
    /* Setup decode shader */
    RET(init_decode_shader(avctx, &ctx->s, &ctx->exec_pool, spv, &prv->decode[0], 0));
    RET(init_decode_shader(avctx, &ctx->s, &ctx->exec_pool, spv, &prv->decode[1], 1));
    RET(init_idct_shader(avctx, &ctx->s, &ctx->exec_pool, spv, &prv->idct[0], 0));
    RET(init_idct_shader(avctx, &ctx->s, &ctx->exec_pool, spv, &prv->idct[1], 1));
